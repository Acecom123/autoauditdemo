...
def show_dashboard():
    ...
    with st.expander("Pending Audit Observations as per Checklist",expanded=True):
        ...
        if not df_pendingObservations.empty:
            df_summ=df_pendingObservations.groupby('Audit_Area',as_index=False)['Criteria'].count()
            col1, col2= st.columns(2)
            with col1:
                st.success("Number of Pending Observations")
                st.bar_chart(df_summ,x='Audit_Area',y='Criteria')
    ...
    with mainSection:
        ...
        if master_options=="DashBoard":
            ...
            with st.form("New User",clear_on_submit=True):
                ...
                userid = st.text_input (label="Enter user ID*", value="", placeholder="User ID Required",key="ck5")
