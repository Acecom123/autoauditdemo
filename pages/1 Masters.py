    ...
    with masters:
        def fileupload(uploaded_file,ftype,table_name,addnew):
                if uploaded_file is not None:
                    dataframe=None
                    if ftype=='CSV':
                        dataframe = pd.read_csv(uploaded_file,encoding= 'unicode_escape',skiprows=header_row-1)
                        ...
                        password=st.text_input("Enter passord*",key="sqlpass")
                        ...
                        jLoad=None
                        payloadtype=st.radio("Is Payload Text or {Dictionory}",options=("{Dictionory}","Text"),key="pods",horizontal=True)
                        payload = st.text_area("Enter Payload",key="apipayload",placeholder="text must be in { curly brackets}")
                        #payload=str(payload).strip("'<>() ").replace('\'', '\"')
                        if payload: 
                            if payloadtype=="{Dictionory}":
                        ...
                        dataframe = pd.read_excel(uploaded_file,skiprows=header_row-1)
                        #st.dataframe(dataframe)
        ...
        if master_options=='Add New Data Set':
            ...
            ftype=st.radio("Select Data Source",options=['CSV','XLSX','XML','JSON','SQL','API'],horizontal=True,
                           help="Import Data from file types like- CSV,XLSX,XML,JSON or Get by- SQL Query or API",key="opcrds")
            if ftype=='CSV':
            ...
            if auditee.empty:
                ...
                substring=["'",'"',':','.','+','-',',',';','`','~','@','#','!','$','%','^','&','*','(',')','[',']','{','}','/','<','>','?']
                spchr=True          
                table_name=st.text_input("Enter Name of Data Set*",key="tablename1",placeholder="Required; Must be Unique; Press Enter to Move ahead ",help=f"Following chatarters not allowed{substring}")
                if table_name:
                ...
                if spchr==True:
                    t="Special Charaters except _ not allowed"
                    new_title=f'<p style="color:Red;">{t}</p>'
                    st.markdown(new_title, unsafe_allow_html=True)

                else:
                        person_responsible=st.selectbox("Select Auditee, who will answer Queries",auditee,key="sbperson_responsibleok")
                        if ftype=='CSV':
                            ...
                            fileupload(uploaded_file,ftype,table_name,"new")                        
                        elif ftype=='JSON':
                ...
                ftype=st.radio("Select File Type",options=['CSV','XLSX','XML','JSON','SQL','API'],horizontal=True,
                           help="Import Data from file types like- CSV,XLSX,XML,JSON or Get by- SQL Query or API",key="opaddcrds")
                ...
                ds_names=get_dsname(int(st.session_state['AuditID']))
                if ds_names.empty:
                    st.success(f"No Data Sets for Current Company...\n First Add Data Set in current Company.")
                else:
                        ...
                        with col2:
                            ...
                            if uploaded_file is not None:
                                    ...
                                    if ftype=='CSV':
                                        ...
                                        jLoad=None
                                        payloadtype=st.radio("Is Payload Text or {Dictionory}",options=("{Dictionory}","Text"),key="pods3",horizontal=True)
            ...
            if ds_name=="---":
                ...
                optionssampl=st.radio("",("Add Sampling","Remove Sampling"),horizontal=True,key="optionfirst")
            ...
            if crud=='Add New':
                ...
                if auditee.empty:
                    ...
                    add_options= st.radio("",('Import Default Check List','Import Check List from Tamplet', 'Add Check List Manually'),horizontal=True,key='op1')
                    ...
                    elif  add_options=='Import Check List from Tamplet': 
                        #download excel tamplet
                        st.success("**Download Excel Tamplet**")
                        with open("checklist.xlsx", 'rb') as f:
                            st.download_button('Download Tamplet',f,"tamplet.xlsx",key="dwnldtam")    
                        
                        #upload tamplet 
                        st.success("**Upload Excel Tamplet**")  
                        #st.error("Upload Excel Tamplet...****")
                        ...
                        if uploaed_file is not None:
                            try:
                                ...
                                if df.columns.tolist()==['Criteria','Audit_Area','Heading']:
                                    cmdok=st.button("Import Tamplet",key="imprttamplet")
                                    if cmdok:
                                        df['Person_Responsible']=person_responsible
                                        df['Audit_id']=auditid
                                        res=import_defalut_checklist(df)
                                    ...
                                    st.error("Check Excel sheet should be Exactly as per Tamplet, with Columns- 'Criteria', 'Audit_Area', 'Heading'")
                            except ValueError:
                ...
                if auditee.empty:
                    ...
                    if selected:
                            ...
                            with placeholder.form("Modify Checklist Doc",clear_on_submit=True):
                                ...
                                mrisk_weight=st.slider("Update Risk Weights",min_value=1,max_value=10,key='slider2rwm',value=selected[0]['Risk_Weight'])
            ...
            with st.expander("View Risk Weights"):
                ...
                original_title = f'<p style="color:Red; ">{mes}</p>'
                st.markdown(original_title, unsafe_allow_html=True)
                mes="2)Check...Data Structure is Excatly same, with same colum names"
                original_title = f'<p style="color:Red; ">{mes}</p>'
                st.markdown(original_title, unsafe_allow_html=True)
                #st.success("1)Upload Current Version of Dataset...\n2)Check...Data Structure is Excatly same, with same colum names")
                #st.success("Check ...Data Structure is Excatly same, with same colum names")
                ftype=st.radio("Select Data Source",options=['CSV','XLSX','XML','JSON','SQL','API'],key="rdio",horizontal=True)
                ...
                if uploaded_file is not None:
                    dataframe_new=None
                    if ftype=='CSV':
                        dataframe_new = pd.read_csv(uploaded_file,encoding= 'unicode_escape',skiprows=header_row-1)
                    elif ftype=='JSON':
                        data = json.load(uploaded_file)
                        ...
                        dataframe_new = pd.read_excel(uploaded_file,skiprows=header_row-1)
