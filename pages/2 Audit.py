    ...
    with cont_observation:
        ...
        if crud=='Update Audit Observations':
            ...
            if selected:
                ...
                st.markdown("""---""")    
                if selected[0]['Compliance_Status']=="Open":
                    with st.expander("Update Observations..."): 
                        with st.form("Modify Observation Doc",clear_on_submit=True):
                            ...
                            if selected[0]['DeadLine']!=None:
                                DeadLine=st.date_input('Update DeadLine',key='DeadLine',value=datetime.strptime(selected[0]['DeadLine'], '%Y-%m-%d').date())
...
def show_planning():
    with  con_show_audit:
        st.info('**Audit Planing & Task Management**')
        crud=st.radio("",('View','Add New','Modify','Delete'),horizontal=True,key='strcrudpl')
        auditid=int(st.session_state['AuditID'])
        ...
        if crud=='Add New':
            ...
            if selected:
                if st.session_state['Role']=="Manager" or st.session_state['User']==selected[0]['created_by']:
                    
                    st.success("Enter details to Modify Selected Record...")
                    with st.form("tasksupdateta",clear_on_submit=True):
                        roid=selected[0]['id']
                        task=st.text_input("Update Task*",key="tasktiu",value=selected[0]['Task'])
                        details=st.text_area("Update Task Details",key="taskdtiu",value=selected[0]['Details'])
                        person=st.selectbox("Update Person Responsible",options=users,key="seusersu")
                        if selected[0]['Due_Date']!=None:
                            duedate=st.date_input('Update Due_Date',key='Due_Date',value=datetime.strptime(selected[0]['Due_Date'], '%Y-%m-%d').date())
    ...
    with cont_oaudit_summary:
        ...
        if crud=='Add New':
                    ...
                    with st.form("Add AduSumm docs",clear_on_submit=True):
                        ...
                        Need_for_Management_Intervention=st.text_input("Need_for_Management_Intervention",key='Need_for_Management_Intervention',value=1)
            ...
            if selected:
                ...
                with st.form("Mdify Summ",clear_on_submit=True):
                        ...
                        risk_weight=st.slider("Set Risk Weights",min_value=1,max_value=10,key='slider2rwsm',value=selected[0]['Risk_Weight'])
                        if risk_weight >=1 and risk_weight <=3:
                                    risk_category='Low'
                        elif risk_weight >=4 and risk_weight <=7:
                                    risk_category='Medium'
            ...
            if len(df) !=0:
                ...
                flist=df.columns
                fileds=st.multiselect("Select Fields to be shown in Report", options=flist,key="fiedsinreport1",)
...
def show_audit():
    ...
    with audit_container:
        ...
        with st.sidebar.markdown("# Audit"):
            sel_option= st.radio("Select Option",('Planning','Documents & Info.','Audit- Vouching, Verification, Review','Observations for Audit Check List','Audit Report Summary','Close Audit'),key='raiodmainop')
        if sel_option=='Documents & Info.':
        ...
        elif sel_option=='Audit Report Summary':
            show_Audit_summary()
            ...
            if d_sname=="---":
                ...
                optionssampl=st.radio("",("Vouching & Verification","Analytical Review & Other Reviews"),horizontal=True,key="optionfirstaudit")
                        ...
                        with placeholder.container():
                            with st.form("Auditing",clear_on_submit=True):
                                ...
                                #with colum1:
                                    ...
                                    audited=AgGrid(selected_df, gridOptions=go_audit,update_mode=(GridUpdateMode.VALUE_CHANGED|GridUpdateMode.SELECTION_CHANGED),columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS)
                                    #st.write(veri
                ...
                if optionssampl=="Analytical Review & Other Reviews":
                #with tab2:
                    ...
                    with st.expander('Analyse Data Set'):
                        ...
                        builder.configure_default_column(groupable=True)
                        builder.configure_pagination(enabled=True,paginationPageSize=25,paginationAutoPageSize=False)
                        go = builder.build()
                        AgGrid(ds,gridOptions=go,columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS)
