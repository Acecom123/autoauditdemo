...
from functions import get_pending_obser,get_all_obser,update_password,get_pending_advere_obser,get_Summary_audit_values,get_ar_queries,get_vv_quries,get_values_id_dsn,get_audit_values,get_queries,get_ar,get_pending_queries,get_dataset_values
...
from functions import get_ar_summary,get_audit_observations,get_Summary_audit_values_comp,get_Summary_audit_values_riskweight,create_user,check_login,assign_user_rights,create_company,get_company_names
...
def show_report():
    ...
    with report_container:
        ...
        with st.sidebar.markdown("# Select..."):
            sta_opts=st.radio('Reports',('Vouching, Verification, Review','Observations by Audit Checklist'),key='ropcompr')
        ...
        if sta_opts=='Vouching, Verification, Review': 
            ...
            if ds=="---":
                ...
                reports=st.radio("",("Reports by Data Set ","Query List"),horizontal=True,key='strcrudasreport')
                ...
                if reports=="Reports by Data Set ":
                    ...
                    with st.expander("Voching & Verification Queries"):
                        ...
                        queriesdf=get_vv_quries(f"{st.session_state['AuditID']}_{ds}",ds,int(st.session_state['AuditID']))
                ...
                #get AR queries lit
                    ...
                    with st.expander("Analytical Reviw & Other Remarks"):
                        ...
                        go = builder.build()
                                    #uses the gridOptions dictionary to configure AgGrid behavior.
                        AgGrid(queries_ar, gridOptions=go,update_mode= (GridUpdateMode.SELECTION_CHANGED|GridUpdateMode.MODEL_CHANGED),columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS)
            ...
            if not df_pendingObservations.empty:
                reports2=st.radio("",("Observations Summary ","Observations List"),horizontal=True,key='obsvrb')
                ...
                #with st.expander("View All Audit Observations"):
                    ...
                    go = builder.build()
                                            #uses the gridOptions dictionary to configure AgGrid behavior.
                    AgGrid(df_allObservations, gridOptions=go,update_mode= (GridUpdateMode.SELECTION_CHANGED|GridUpdateMode.MODEL_CHANGED),key="sum14",columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS)
                    st.success(f"""Use Options to Group Report by Mutiple Levels.
                                                    Right Click to Export Report to Excel""")
                    st.info("**Download Report in Word**")
                    
                    flist=df_allObservations.columns
                    fileds=st.multiselect("Select Fields to be shown in Report", options=flist,key="fiedsinreport",)
                    
                    if fileds:
                        wordrep=st.button("Generate & Download Report",key="wrodrep")
                        if wordrep:
                            df_allObservations=df_allObservations[fileds]
                            st.dataframe(df_allObservations)
